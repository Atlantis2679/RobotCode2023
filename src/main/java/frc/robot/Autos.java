...
public final class Autos {
    ...
    public static Command releaseConeThird(Arm arm, Intake intake, Drivetrain drivetrain) {
        return new DriveToDistance(drivetrain, releaseConeThirdConstants.DRIVE_TO_DISTANCE_START)
                ...
                .andThen(releaseCone(intake))
                .andThen(new DriveToDistance(drivetrain, releaseConeThirdConstants.DRIVE_TO_DISTANCE_END))
                .andThen(ArmPositionsCommands.rest(arm)
    ...
    public static Command driveBackwardsOutsideCommunity(Drivetrain drivetrain, boolean turnOnFinish) {
        ...
        return turnOnFinish
                ? driveToDistanceCommand.withTimeout(DriveBackwardsOutsideCommunityConstants.TIMEOUT_SECONDS)
                        .andThen(
                                new TurnByDegree(drivetrain, DriveBackwardsOutsideCommunityConstants.TURN_ENGLE))
                : driveToDistanceCommand.withTimeout(DriveBackwardsOutsideCommunityConstants.TIMEOUT_SECONDS);
    }
    ...
    public static Command balanceChargeStation(Drivetrain drivetrain, Arm arm, boolean isReversed) {
        return new InstantCommand(() -> {
            ...
            arm.setVoltageElbow(0);
                ...
                new GetOnChargeStation(drivetrain, isReversed).withTimeout(GetOnChargeStationConstants.TIMEOUT_SECONDS))
                .andThen(() -> drivetrain.setSpeed(-BalanceOnChargeStationConstants.SPEED_TO_CLOSER_CENTER,
